...
const GameCard = React.memo<GameCardProps>(({ id, title, category, color, icon, description }) => {
  ...
  // Use granular selectors to prevent this component from re-rendering
  // when unrelated store updates happen (like stats, xp, etc).
  // This component will now ONLY re-render if:
  // 1. Its specific props change (handled by React.memo)
  // 2. Its specific favorite status changes
