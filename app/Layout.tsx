...
const Layout = () => {
  ...
  return (
    <div className="min-h-screen flex bg-light-bg dark:bg-dark-bg text-gray-900 dark:text-gray-100 font-sans transition-colors duration-300 selection:bg-blue-500/30">
      ...
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex flex-col w-72 fixed h-full border-r border-gray-200 dark:border-white/5 bg-white/50 dark:bg-[#09090b]/80 backdrop-blur-xl z-50">
      ...
      {/* Mobile Header */}
      <header className="md:hidden fixed top-0 left-0 right-0 h-16 border-b border-gray-200 dark:border-white/5 bg-white/80 dark:bg-[#09090b]/80 backdrop-blur-md z-40 flex items-center justify-between px-4">
      ...
      {/* Mobile Bottom Nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-[#09090b]/90 backdrop-blur-lg border-t border-gray-200 dark:border-white/5 flex justify-around items-center px-2 z-50 pb-[env(safe-area-inset-bottom)] pt-2 h-[calc(60px+env(safe-area-inset-bottom))]">
         <NavItem to="/" icon={Home} label="Home" />
